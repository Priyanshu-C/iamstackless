Normalised images for the watches drawer.

Do not add files here by hand. Stage the raw photo in
`scripts/incoming/watches/<item-id>.<ext>` and run:
...
The item id must match `lib/collections/watches.ts`. The data-integrity test
